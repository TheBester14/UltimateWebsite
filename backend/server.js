...
//     origin: "https://ultimatewebsite2.onrender.com/",
//     methods: ["GET", "POST"],
...
//   "http://localhost:3000",
//   "https://ultimatewebsite2.onrender.com/",
// ];
...
const io = socketIo(server, {
  cors: {
    origin: ["http://localhost:3000", "https://ultimatewebsite2.onrender.com"],
    methods: ["GET", "POST"],
    allowedHeaders: ["Authorization"],
    credentials: true,
  },
...
// MongoDB connection string (now using environment variables)
const uri = process.env.MONGODB_URI; // Using an environment variable for MongoDB URI
const client = new MongoClient(uri, {
  serverApi: {
    version: ServerApiVersion.v1,
    strict: true,
...
const authenticateToken = (req, res, next) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET, (err, user) => {
    // Using env variable for JWT secret
    if (err) return res.sendStatus(403);
    req.user = user;
    next();
  });
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { id: user._id, username: user.username },
      process.env.JWT_SECRET, // Use env variable for JWT secret
      { expiresIn: "1h" }
    );
    res.send({ message: "Login successful", token });
  } catch (err) {
